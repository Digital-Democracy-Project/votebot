"""Bill votes lookup service.

This service provides real-time vote lookup for bills via the OpenStates API.
"""
...
class BillVotesService:
    """
    Service for fetching bill votes from OpenStates.

    Always fetches fresh data from the OpenStates API to ensure
    structured vote records (BillVote/VoteRecord) are available
    ...
    async def get_bill_votes(
        ...
        """
        Get votes for a bill directly from OpenStates.
        ...
        """
        logger.info(
            "Fetching bill votes from OpenStates",
            jurisdiction=jurisdiction,
            session=session,
            bill_identifier=bill_identifier,
        )

        return await self._fetch_from_openstates(jurisdiction, session, bill_identifier)
    ...
    async def lookup_legislator_vote(
        ...
        Uses get_bill_info() which always fetches fresh from OpenStates
        and includes session fallback for state bills.
        ...
        Returns:
            Dict with vote info or None if not found
        """
        result = await self.get_bill_info(jurisdiction, session, bill_identifier)
        ...
        return self.find_legislator_in_votes(
